...
export function ProductModal({ product, open, onClose }) {
  ...
  const productPrice = product.isDiscounted && product.discountPrice ? product.discountPrice : product.price;
        ...
        <div className="max-h-[calc(100dvh-1.5rem)] overflow-y-auto">
          <div className="px-5 pb-5 pt-14 sm:px-6">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <div className="rounded-[18px] border px-4 py-3 text-right text-sm font-semibold text-[var(--site-muted)]" style={{ borderColor: 'var(--site-border)', background: 'rgba(255,255,255,0.04)' }}>
                <div className="text-xs text-[var(--site-muted)]">{lang === 'ar' ? 'متوسط الانتظار' : 'Average wait time'}</div>
                <div className="mt-1 text-base font-bold text-[var(--site-text)]">{product.averageWaitTime ?? 0} {lang === 'ar' ? 'دقيقة' : 'min'}</div>
              </div>
            </div>
